...
##### NOTE: As of Elements v.17, getnewaddress defaults to creating P2SH-P2WPKH addresses. You can create 'CTE' prefixed addresses (as generated by Elements pre-0.17) by using the addresstype=legacy argument on node startup, or by setting it in your config file. Some commands require a 'legacy' style address in order to work, such as message signing, as shown in the [Advanced examples]({{ site.url }}/elements-code-tutorial/advanced-examples) section.
